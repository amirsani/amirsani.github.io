...
const ReturnSimulation = () => {
  ...
  return (
    <div className="w-full max-w-4xl mx-auto bg-white rounded-lg shadow-lg p-6">
      <div className="mb-4">
        <h2 className="text-2xl font-bold mb-2">European VC Returns Distribution</h2>
      </div>
        ...
        {stats.returns.map((count, i) => {
          ...
          return (
            <div key={i} className="text-center">
              <div className="font-bold">{binLabels[i]}</div>
              <div className="text-lg">{percentage.toFixed(1)}%</div>
      ...
      <svg width={width} height={height} className="bg-black rounded-lg">
        ...
        {stats.returns.map((count, i) => {
          ...
          return (
            ...
            >
              <title>
                {`${(count / Math.max(1, stats.balls) * 100).toFixed(1)}%`}
              </title>
            </rect>
          );
        })}
        ...
        {binLabels.map((label, i) => {
          ...
          return (
            <text
              ...
              fontSize="12"
            >
              {label} ({(binWidths[i] * 100).toFixed(0)}%)
            </text>
          );
        })}
      </svg>
